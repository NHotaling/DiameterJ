...
// Creates custom file names for use later
			var name0= getTitle;
			var name1= getTitle+"_Intersections.csv";	
...
//				saveAs("Results",path1);
//				run("Close");
//					pathfile= myDir2+name1; 
//						filestring=File.openAsString(pathfile); 
//						rows1=split(filestring, "\n"); 
//						c1=newArray(rows1.length); 
//						sx_coord = newArray(rows1.length); 
//						sy_coord = newArray(rows1.length);
//						fx_coord = newArray(rows1.length);
//						fy_coord = newArray(rows1.length);
//						Euclid_Dist = newArray(rows1.length);
//						Branch_Length = newArray(rows1.length);
//							for(v=0; v<rows1.length; v++) 	{
//								columns=split(rows1[v],","); 
//								c1[v]=parseInt(columns[2]);
//								if (c1[v] > 9) {	
//									sx_coord[v]=parseInt(columns[3]);
//									sy_coord[v]=parseInt(columns[4]);
//									fx_coord[v]=parseInt(columns[6]);
//									fy_coord[v]=parseInt(columns[7]);
//									Branch_Length[v]=parseInt(columns[2]);
//									Euclid_Dist[v]=parseInt(columns[9]);
//							}} 
//		Array.show("Intersection Info",sx_coord,sy_coord,fx_coord,fy_coord,Branch_Length,Euclid_Dist);
//		selectWindow("Intersection Info");
//			saveAs("Results",path1);
//				run("Close");
//			open(myDir2+name1);
//				run("Summarize");
//					Char_Length_Mean = getResult("Branch_Length",nResults-4);
//					Char_Length_SD = getResult("Branch_Length",nResults-3);
//					Char_Length_Max = 
//				saveAs("Results",path1);
//				run("Close");
...
//Performs a correction for overestimation of fiber length because of intersections
				...
				Corr_Medial_Fiber_length= CMedial_Len;
				CMFiber_Diam= c;
